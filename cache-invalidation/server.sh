...
then
	...
	replica 2 \
	$(hostname --fqdn):/bricks/${GLUSTER_VOLUME}/b{1,2} $4:/bricks/${GLUSTER_VOLUME}/b{1,2} force

    gluster volume start ${GLUSTER_VOLUME} force
    
    # cache-invalidation on for the volume
    gluster v set ${GLUSTER_VOLUME} features.cache-invalidation on
fi
