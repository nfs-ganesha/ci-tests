...
then
	...
	line=$(grep -n "Pseudo" ${conf_file} | sed 's/^\([0-9]\+\):.*$/\1/')
	sed -i "$line s/${GLUSTER_VOLUME}/ppath/" ${conf_file}
	sed -i "s/\"3\"/\"4\"/" ${conf_file}
...
then
	...
	sed -i "s/No_root_squash/root_squash/" ${conf_file}
	sed -i "s/\"4\"/\"3\", \"4\"/" ${conf_file}
